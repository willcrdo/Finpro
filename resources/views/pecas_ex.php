<?php
	class Computador{
		...
		$sqlQuery = "INSERT INTO ". $this->db_table ." SET conteudo = :conteudo";
		$stmt = $this->conn->prepare($sqlQuery);
		$stmt->execute();
		...
		public function getPecaUnica(){
			$sqlQuery = "SELECT item_id, conteudo FROM ". $this->db_table ." WHERE item_id = ? LIMIT 0,1";
			$stmt = $this->conn->prepare($sqlQuery);
		...
		public function updatePeca(){
			$sqlQuery = "UPDATE ". $this->db_table ." SET conteudo = :conteudo WHERE item_id = :item_id";
			$stmt = $this->conn->prepare($sqlQuery);
			$this->conteudo=htmlspecialchars(strip_tags($this->conteudo));
